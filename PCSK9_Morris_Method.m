...
P = cell2mat(struct2cell(parameters));

ScaledPoints = ScaleCreate(M, P);

%Set up for complex step gradient approximation
h = 1e-14;
MorrisScores = zeros(M, length(P));

%Try using finite differences (Compare)
%Plot mu vs sigma on one plot as a scatter plot
...
for j = 1:length(P)
    for i = 1:M
        ...
        P(j) = complex(P(j), h);
        rowDist = [1*ones(1,45)];
        CP = mat2cell(P, rowDist);
        parameters = cell2struct(CP, fields);
        ...
        Y = [Y1; Y2; Y3; Y4; Y5];
        T = [T1; T2; T3; T4; T5];
        MorrisScores(i, j) = imag(Y(550,2)) / h; %USER EDIT: Desired output
        %Consider integrating this value to get all value up until the time
        %point chosen. Or since last dose. Compare with current setup
        P(j) = real(P(j));
        P = P ./ ScaledPoints(i,:)';
    end
end

djSj = zeros(2, length(P));

for i = 1:length(P)
    djSj(1,i) = mean(abs(MorrisScores(: , i)));
    djSj(2,i) = std(MorrisScores(: , i));
end

plotfields = string(fieldnames(parameters))';
[pV, pI] = maxk(djSj(1,:), 10);
str = string(pI);
xlabPoints = djSj(1,pI);
xlabPoints = xlabPoints - 100;
ylabPoints = djSj(2,pI);
ylabPoints = ylabPoints - 550;

% Plot dj and Sj
figure;
scatter(djSj(1,:), djSj(2,:), 40, 'k')
title('Morris Method Results', 'FontSize', 26)
ylabel('Standard Deviation', 'FontSize', 22)
xlabel('Absolute Value of Mean', 'FontSize', 22)
hold on
refline(1,0)
text(xlabPoints, ylabPoints, str)
ax = gca;
properties(ax);
ax.LineWidth = 1.2;
set(ax, 'FontSize', 18)
