...
I2 = [24, 25];
LatinPoints = lhsdesign(M, 2);
ScaledPoints = zeros(M, 2);
for i = 1:M
    for j = 1:2
        ScaledPoints(i, j) = (((pmaxs(I2(j)) - pmins(I2(j))) * LatinPoints(i, j)) + pmins(I2(j)));
    end
end

for i = 1:M
    for j = 1:2
        P{I2(j)} = P{I2(j)} * ScaledPoints(i, j);
    end
...
I4 = [24, 25, 7, 35];
LatinPoints = lhsdesign(M, 4);
ScaledPoints = zeros(M, 4);
for i = 1:M
    for j = 1:4
        ScaledPoints(i, j) = (((pmaxs(I4(j)) - pmins(I4(j))) * LatinPoints(i, j)) + pmins(I4(j)));
    end
end

for i = 1:M
    for j = 1:4
        P{I4(j)} = P{I4(j)} * ScaledPoints(i, j);
    end
    parameters = cell2struct(P, fields);
    IC = getInitialConditions();
    t0 = 0; td1 = 28; td2 = 56; td3 = 84; td4 = 112; tfinal = 140;
...
I6 = [24, 25, 7, 35, 3, 26];
LatinPoints = lhsdesign(M, 6);
ScaledPoints = zeros(M, 6);
for i = 1:M
    for j = 1:6
        ScaledPoints(i, j) = (((pmaxs(I6(j)) - pmins(I6(j))) * LatinPoints(i, j)) + pmins(I6(j)));
    end
end

for i = 1:M
    for j = 1:6
        P{I6(j)} = P{I6(j)} * ScaledPoints(i, j);
    end
    parameters = cell2struct(P, fields);
    IC = getInitialConditions();
    t0 = 0; td1 = 28; td2 = 56; td3 = 84; td4 = 112; tfinal = 140;
...
I8 = [24, 25, 7, 35, 3, 26, 41, 5];
LatinPoints = lhsdesign(M, 8);
ScaledPoints = zeros(M, 8);
for i = 1:M
    for j = 1:8
        ScaledPoints(i, j) = (((pmaxs(I8(j)) - pmins(I8(j))) * LatinPoints(i, j)) + pmins(I8(j)));
    end
end

for i = 1:M
    for j = 1:8
        P{I8(j)} = P{I8(j)} * ScaledPoints(i, j);
    end
    parameters = cell2struct(P, fields);
    IC = getInitialConditions();
    t0 = 0; td1 = 28; td2 = 56; td3 = 84; td4 = 112; tfinal = 140;
...
figure;
title('Active Subspace Reduced Models vs Full Model PDFs', 'FontSize', 24)
tiledlayout(2,2)
nexttile
[fTest, xTest] = ksdensity(OutsTest2, 'support', 'positive');
...
ax.LineWidth = 1.1;
set(ax, 'FontSize', 16)
legend('Top 2 AS Selected Parameters', 'Full Range', 'FontSize', 18)
xlabel('LDLc Level (mg)', 'FontSize', 20)
ylabel('Probability', 'FontSize', 20)
nexttile
[fTestM, xTestM] = ksdensity(OutsTest4, 'support', 'positive');
...
ax.LineWidth = 1.1;
set(ax, 'FontSize', 16)
legend('Top 4 AS Selected Parameters', 'Full Range', 'FontSize', 18)
xlabel('LDLc Level (mg)', 'FontSize', 20)
ylabel('Probability', 'FontSize', 20)
nexttile
[fTestG, xTestG] = ksdensity(OutsTest6, 'support', 'positive');
...
ax.LineWidth = 1.1;
set(ax, 'FontSize', 16)
legend('Top 6 AS Selected Parameters', 'Full Range', 'FontSize', 18)
xlabel('LDLc Level (mg)', 'FontSize', 20)
ylabel('Probability', 'FontSize', 20)
nexttile
[fTest8, xTest8] = ksdensity(OutsTest8, 'support', 'positive');
plot(xTest8, fTest8, 'color', [0.6350, 0.0780, 0.1840], 'LineWidth', 3)
hold on
[fFull, xFull] = ksdensity(CompOuts, 'support', 'positive');
plot(xFull, fFull, 'color', [0, 0.4470, 0.7410], 'LineWidth', 3) %Full Model Color
hold off
...
ax.LineWidth = 1.1;
set(ax, 'FontSize', 16)
legend('Top 8 AS Selected Parameters', 'Full Range', 'FontSize', 18)
xlabel('LDLc Level (mg)', 'FontSize', 20)
ylabel('Probability', 'FontSize', 20)
%legend('Active Subspace Selected Parameters', 'Morris Method Selected Parameters', 'Gaussian Process Selected Parameters', 'Full Range', 'FontSize', 20)
...
%title('Gaussian Process vs Full Model PDF, Dose = 4w400', 'FontSize', 26)
