addpath('functions');
rng(27);
%For each output, go to 20 on activity scores and range parameters
%Try vectored output of all 3 outputs both separately and together
%randn to fix flat spots vector size scaled and then add to output
SigPars = 5;
M = 10*SigPars; %Should be about 10 * dimensions
%Try picking the parameters from the range parameters from the other GP
%results
%[V,I] = maxk(FULLactivityScores{8}(:,25), SigPars);
I = [35 24 7 40 22];
parameters = antiPCSK9modelPars();
fields = fieldnames(parameters);
P = struct2cell(parameters);

%5 percent for unknowns, 1 for held constants
...
testN = 100;
TestingPoints = lhsdesign(testN, SigPars);
%Run actual model at these points to compare to predictive model

options.nugget_est = true;
model = ppgasp(LatinPoints, Outs', options);
pred_model = predict_ppgasp(model, TestingPoints);

TestingScaledPoints = zeros(testN, SigPars);
TrueOuts = cell(1, testN);

for i = 1:testN
    for j = 1:SigPars
        TestingScaledPoints(i, j) = (((pmaxs(I(j)) - pmins(I(j))) * TestingPoints(i, j)) + pmins(I(j)));
    end
end

for i = 1:testN
    for j = 1:SigPars
        P{I(j)} = P{I(j)} * TestingScaledPoints(i, j);
    end
    parameters = cell2struct(P, fields);
    IC = getInitialConditions();
    t0 = 0; td1 = 28; td2 = 56; td3 = 84; td4 = 112; tfinal = 140;
...
TrueOuts = cell2mat(TrueOuts);
TrueOuts = TrueOuts';
errors = (pred_model.upper95(:,:) - pred_model.lower95(:,:))/2;
PlotVect = [TrueOuts errors pred_model.mean(:,:)];
PlotVect = sortrows(PlotVect);
figure;
scatter((1:100), PlotVect(:,1), 20, 'r', 'filled', 'DisplayName', 'True Values')
xlabel('Test Index', 'FontSize', 22)
ylabel('LDLc Level (mg)', 'FontSize', 22)
title('Gaussian Process Fit Test', 'FontSize', 26)
hold on
errorbar(1:100, PlotVect(:,3), PlotVect(:,2), 'b.', 'DisplayName', 'GP 95% CI');
...
% for i = 1:6
%     nexttile
%     plot(pred_model.mean(s*i,:), 'r', 'DisplayName', 'Model Mean');
%     hold on
%     plot(TrueOuts(s*i,:), 'k', 'DisplayName', 'True Value');
%     plot(pred_model.upper95(s*i,:), '--b', 'HandleVisibility', 'off');
%     plot(pred_model.lower95(s*i,:), '--b', 'HandleVisibility', 'off');
%     legend('Location','Best');
%     hold off
% end
% 
% checkMat = TrueOuts >= pred_model.lower95 & TrueOuts <= pred_model.upper95;
% pInCI = sum(checkMat, 'all') / numel(checkMat);
