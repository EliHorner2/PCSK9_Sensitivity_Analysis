...
%at each parameter for each row (set of input points)
    for i = 1:M
        for j = 1:length(P)
            ...
            Y = [Y1; Y2; Y3; Y4; Y5];
            T = [T1; T2; T3; T4; T5];
            gradApprox(j, i) = imag(Y(550,2)) / h; %USER EDIT: Desired output
            P{j} = real(P{j});
...
DS = diag(S);
FULLactivityScores = cell(1, length(P));

for n = 1:length(P)
    activityScores = zeros(length(P), 1);
    for i = 1:length(P)
        for j = 1:n
            activityScores(i) = activityScores(i) + DS(j) * (U(i,j))^2;
        end
    end
    FULLactivityScores{n} = activityScores;
end
...
%Plot Activity Scores vs PRCC
figure(1);
% tiledlayout(2,1);
% ax1 = nexttile;
bar(ltFAS(:,25))
xticks(1:length(plotfields))
xticklabels(plotfields)
xtickangle(30)
ylabel('Log of Activity Score', 'FontSize', 22)
title('LDLc Active Subspace Results', 'FontSize', 26)
%hold on
% ax2 = nexttile;
% plotSABarResults(S_OUTPUTS(1).prcc(1:end-1, end),parsName,OUTPUTNames(1),'_prccBar.png')
...
xticklabels(plotfields)
xtickangle(30)
ylabel('Log of Activity Score', 'FontSize', 18)
title('LDLc Active Subspace Results', 'FontSize', 24)
nexttile;
bar(PRCCVals)
xticks(1:length(plotfields))
xticklabels(plotfields)
xtickangle(30)
ylabel('|PRCC|', 'FontSize', 18)
title('LDLc PRCC Results', 'FontSize', 24)

figure;
scatter(1:45,log10(diag(S)))
title('Singular Values')
title('Singular Values', 'FontSize', 26)
xlabel('Index', 'FontSize', 22)
ylabel('Log of Magtinude of Singular Value', 'FontSize', 22)
hold on
yline(-1)

%Plot first column of U (called W in paper)
%U columns are uncentered principal directons from the gradient evaluations
% figure(1);
% tiledlayout(2,1);
% nexttile
% bar(abs((U(:,1))))
